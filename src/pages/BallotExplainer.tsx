...
const BallotExplainer: React.FC = () => {
  ...
  const handleExplain = () => {
    ...
    setTimeout(() => {
      setExplanation({
        summary: "This measure would increase the local sales tax by 0.5% to fund public transportation improvements, specifically the expansion of the light rail system.",
        yesMeans: "A YES vote supports increasing the sales tax by 0.5% to fund transit.",
        noMeans: "A NO vote opposes the tax increase, leaving funding for transit at current levels."
      });
          ...
          <Card className="h-full flex flex-col shadow-lg border-0">
            <CardContent className="p-8 flex-1 flex flex-col">
              <div className="flex justify-between items-center mb-4">
                <label className="font-bold text-gray-900 text-lg">Paste Measure Text</label>
          ...
          <Card className={`h-full border-t-4 shadow-lg ${explanation ? 'border-t-[var(--accent)] bg-white' : 'border-t-gray-200 bg-gray-50/50'}`}>
            <CardContent className="p-8 h-full flex flex-col">
              <h3 className="font-bold text-2xl text-gray-900 mb-6 flex items-center gap-3 font-display">
                <FileText className="w-7 h-7 text-[var(--accent)]" /> Plain English Translation
              </h3>
